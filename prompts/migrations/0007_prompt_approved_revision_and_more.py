"""
Beta 11.11B2A: add the review-binding columns to Prompt and clean up the
...
Data
----
Every prompt currently in ``review`` or ``approved`` was moved there before any
...
def clean_unbound_review_states(apps, schema_editor):
    ...
    for pks, target_status in ((to_draft, "draft"), (to_rework, "rework")):
        if not pks:
            continue
        Prompt.objects.using(db_alias).filter(pk__in=pks).update(
