"""
Beta 11.5 groups H/I: the admin change-form entry point, and the guarantee
...
from prompts.tests.draft_preview_fixtures import make_draft_prompt, make_user, publish
...
class DraftPreviewButtonTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.prompt = make_draft_prompt(
            cls.owner, slug="btn-prompt-en", title="Button Prompt"
        )
        cls.prompt.create_translation(
            "de", slug="btn-prompt-de", title="Button Prompt DE", intro="i", body="b", outro="o"
        )
    ...
    def _expected_link(self, language_code):
        return reverse(
            "admin:prompts_prompt_draft_preview", args=[self.prompt.pk, language_code]
        )
    ...
    def test_existing_draft_versus_live_button_is_untouched(self):
        ...
        resp = self.client.get(self._change_url())
        self.assertContains(
            resp, reverse("admin:prompts_prompt_diff", args=[self.prompt.pk])
        )
        self.assertContains(resp, "Draft ↔ Live")


class PreviewStaysOutOfPublicSurfacesTests(TestCase):
    """Group I: the preview URL must never be advertised publicly."""
    ...
    def test_preview_url_is_absent_from_the_sitemap(self):
        prompt = make_draft_prompt(
            self.editor, slug="sitemap-prompt-en", title="Sitemap Prompt"
        )
        ...
        # The ordinary public prompt URL is still advertised.
        self.assertIn("/en/prompts/sitemap-prompt-en/", xml)

    def test_preview_url_is_absent_from_the_search_visibility_queryset(self):
        """search.adapters.prompts.PromptSearchAdapter builds its result set
        from Prompt.objects.visible_in_language(language_code) - the same
        ...
        preview route. Exercised here without PromptSearchAdapter.search()
        itself, which requires PostgreSQL full-text search and is not
        reachable under the SQLite fallback (see the Beta 11.5 final report)."""
        from prompts.models import Prompt

        prompt = make_draft_prompt(
            self.editor, slug="search-prompt-en", title="Search Prompt Marker"
        )
        publish(prompt, self.editor)

        for obj in Prompt.objects.visible_in_language("en"):
