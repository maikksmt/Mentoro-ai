"""
Beta 11.8 group A: the draft-preview route's HTTP contract.

Mirrors ``prompts/tests/test_draft_preview_route.py`` (Beta 11.5) and
``guides/tests/test_draft_preview_route.py`` (Beta 11.4).
"""
...
class DraftPreviewRouteTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.usecase = make_draft_usecase(
            cls.editor, slug="route-usecase-en", title="Route Usecase"
        )
    ...
    def test_valid_get_renders(self):
        resp = self.client.get(preview_url(self.usecase.pk, "en"))
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, "Route Usecase")

    def test_head_is_allowed(self):
        resp = self.client.head(preview_url(self.usecase.pk, "en"))
        self.assertEqual(resp.status_code, 200)

    def test_post_is_rejected_with_405(self):
        resp = self.client.post(preview_url(self.usecase.pk, "en"))
        self.assertEqual(resp.status_code, 405)

    def test_put_patch_and_delete_are_rejected_with_405(self):
        url = preview_url(self.usecase.pk, "en")
    ...
    def test_unknown_object_id_is_404(self):
        resp = self.client.get(preview_url(self.usecase.pk + 9999, "en"))
        self.assertEqual(resp.status_code, 404)

    def test_non_numeric_object_id_is_404(self):
        resp = self.client.get(
            reverse("admin:usecases_usecase_draft_preview", args=["not-a-pk", "en"])
        )
        self.assertEqual(resp.status_code, 404)

    def test_unsupported_language_is_404(self):
        resp = self.client.get(preview_url(self.usecase.pk, "fr"))
        self.assertEqual(resp.status_code, 404)

    def test_missing_translation_is_404_and_never_falls_back(self):
        # The usecase only has an English translation.
        resp = self.client.get(preview_url(self.usecase.pk, "de"))
        self.assertEqual(resp.status_code, 404)
    ...
    def test_no_canonical_link_is_rendered(self):
        html = self.client.get(preview_url(self.usecase.pk, "en")).content.decode()
